...
let number =  23; //number représente uniquement des chiffres entiers et décimaux, positifs ET négatifs, Number est un type de données spécifique au javascript que l'on ne retrouve pas dans les autres langages
...
let string3 = `mon chien fait ${number} cm de long` // Si j'utilise les accents grave pour déclarer une chaine de caractère je dois utiliser un littéral de gabarit pour déclarer l'utilisation d'une variable dans la chaine de caractère

// Cas spécifique d'une variable avec  plusieurs type de données 

let multiData = 'Bonjour' + 42 + true; // dans le cas d'une variable avec plusieurs type de données je dois utiliser la concaténation pour pouvoir les liés, pour cela on utilisera le concaténateur + avec un espace entre les informations qu'il doit concaténer